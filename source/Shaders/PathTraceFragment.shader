...
struct IntersectionData
{
	...
	vec3 point;		// point of intersection in world space
	vec3 normal;	// the normal of the surface at the point of intersection
	vec3 tangent;	// the tangent of the surface
	vec3 bitangent;	// the bitangent of the surface
	uint material;	// the material or the intersected surface
	bool miss;		// if the ray did not intersect anything
};
...
uint pcg_hash(uint inp) // random number generator
{
...
float rand_float(inout uint seed) // generates a float from 0 to 1
{
	seed = pcg_hash(seed);
...
{
	float r = sqrt(rand_float(seed));
	float theta = 2.0 * pi * rand_float(seed);
	return vec2(r * cos(theta), r * sin(theta));
}

vec3 sampleHemisphere(inout uint seed) // generates a random uniformly distribted point on a unit hemisphere
{
	float z = rand_float(seed);
	float r = safe_sqrt(1.0 - z * z);
...
{
	vec2 d = sampleDisk(seed);
	float z = safe_sqrt(1.0 - d.x * d.x - d.y * d.y);
	return vec3(d.x, d.y, z);
}

vec3 sampleMap(vec3 dir) // samples the color of the environment map
{
...
// test intersection between ray and bounding volume
...
void main()
{
	// coordinates of the pixel on screen
	...
	vec3 start = (camera * ray_start).xyz; // start at origin of the camera
	vec3 end = (camera * ray_end).xyz; // going through the pixel
	...
	if (data.miss)
		...
		FragColor = vec4(materials[data.material].albedo, 1.0);
}